...
class Braitenberg():
    ...
    def image_callback(self, img):
        ...
        green_filter = cv2.inRange(cv_image,numpy.array((0, 125, 0)),numpy.array((255, 255, 255)))
                            
        #gray_image = cv2.cvtColor(cv_image, cv2.COLOR_BGR2GRAY)  # Convert to gray scale

        mid_width = img.width / 2;

        left_image = green_filter[:, 0:mid_width]
        right_image = green_filter[:, mid_width:img.width]
        ...
        #twist_msg = self.wheel_motor_power_to_twist_msg(normalised_right_intensity, normalised_left_intensity)
        ...
        global laserScanComplete
        while laserScanComplete == False:
            foo = foo + 1
            turn_angle = 0
            forward_speed = 0
        if tooClose:
    ...
    def laser_callback(self, msg):
        ...
        global tooClose
        tooClose = min_distance < 1        
        
        global laserScanComplete
        laserScanComplete = True
