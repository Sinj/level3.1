...
class Braitenberg():
    ...
    def __init__(self, name):
        ...
        self.dir = 0.0
        self.rate = rospy.Rate(10)  # Set a rate of 10hz
        self.issafe = True       #- spin around at loaction till box seen
        self.ispre = False     #- if see on left : turn 140 deg and run away else :turn -140 deg, set run away to true
...
#                self.rate.sleep()                         # Sleep to ensure rate of 10hz
        else:
            print 'all clear' 
            self.issafe = True
        ...
        if self.issafe == False:
            ...
            if leftim > rightim:
                ...
                self.dir = 3.0
                twist_msg.angular.z = 3.0  
            while (self.timer + 4) > time.time() and not rospy.is_shutdown():
                self.cmd_vel_pub.publish(twist_msg)
                self.rate.sleep()                         # Sleep to ensure rate of 10hz
    ...
    def laser_callback(self, msg):
        ...
        while (self.timer + 30) > time.time() and self.isrun == True and not rospy.is_shutdown():
            if min_distance < 0.85:                    # If the robot is close, then
                #rospy.loginfo("Turn")  # Turn the robot
                
                now = time.time()          
                end_time = now + 1.6                     # Defining for how long the robot should turn.
                angle_velocity = self.dir                     # Defining the angular velocity and direction of the turn. Use np.pi to get PI.
                while end_time >= time.time():                 # While the run time is not up, do
                    ...
                    self.rate.sleep()                         # Sleep to ensure rate of 10hz
            elif min_distance >= 0.85:                                    # If the robot is far away from everything, then
...
#        self.rate = rospy.Rate(10)  # Set a rate of 10hz
#        self.issafe = True       #- spin around at loaction till box seen
#        #self.isseens = False     #- if see on left : turn 140 deg and run away else :turn -140 deg, set run away to true
#        self.isrun = False       #-start timer, if running away for X seconds stop rotate 180          # Creatingan OpenCV bridge object used to create an OpenCV image from the ROS image
#        cv2.namedWindow("Image window", 1)  # Opening a window to show the image
#        cv2.startWindowThread()
#
...
#        
#        
#        while self.isrun == False and not rospy.is_shutdown():
#            print "Normalised sum intensity: ", wholeintensity1
#            print "Normalised mean intensity1: ", wholeintensity        
#            print "Mean intensity left: ", leftim
#            print "Mean intensity right: ", rightim
...
#            print self.issafe
#            
#            if wholeintensity1 > 500: # if pred seen and not runing = change flag
#                print 'robot seen' 
#                self.issafe = False
#                twist_msg.angular.z = 0
#                self.cmd_vel_pub.publish(twist_msg)
...
#                #rospy.loginfo("Turn")  # Turn the robot
#                
#                now = time.time()          # Get the current time as seconds.milliseconds
#                end_time = now + 1.6                     # Defining for how long the robot should turn.
#                angle_velocity = self.dir                     # Defining the angular velocity and direction of the turn. Use np.pi to get PI.
#                while end_time >= time.time():                 # While the run time is not up, do
#                    twist_msg.linear.x = 0                # Set linear speed
